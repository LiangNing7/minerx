...
[[ $(type -t minerx::protoc::loaded) == function ]] && return 0
...
# Generates $1/api.pb.go from the protobuf file $1/api.proto
# $1: Full path to the directory where the api.proto file is
function minerx::protoc::protoc() {
  local package=${1}
  gogopath=$(dirname "$(minerx::util::find-binary "protoc-gen-gogo")")
...
function minerx::protoc::install() {
  ...
  (
    ...
    if [[ ! -f protoc  ]]; then
      ...
      if [[ ${os} == "darwin" ]]; then
        ...
        minerx::log::info "This install script does not support ${os}/${arch}"
        return 1
      fi
    ...
    minerx::log::info "$HOME/bin/protoc v${PROTOC_VERSION} installed."
  )
}

# Marker function to indicate protoc.sh has been fully sourced
minerx::protoc::loaded() {
  return 0
}

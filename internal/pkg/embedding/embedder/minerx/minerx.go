...
// minerxEmbedder implements the Embedder interface and manages concurrent embeddings.
type minerxEmbedder struct {
	concurrency int
	rl          ratelimit.Limiter
	embedder    Embedder
}

// NewEmbedder creates a new instance of *minerxEmbedder with specified options.
func NewEmbedder(typed Embedder, opts ...Option) *minerxEmbedder {
	emb := &minerxEmbedder{
...
func (emb *minerxEmbedder) Embedding(ctx context.Context, inputs []any) ([]string, error) {
	if len(inputs) == 0 {

...
import (
	...
	"github.com/LiangNing7/minerx/cmd/minerx-controller-manager/app/config"
	"github.com/LiangNing7/minerx/cmd/minerx-controller-manager/app/options"
	"github.com/LiangNing7/minerx/cmd/minerx-controller-manager/names"
	configv1beta1 "github.com/LiangNing7/minerx/internal/controller/apis/config/v1beta1"
	"github.com/LiangNing7/minerx/internal/gateway/store"
	"github.com/LiangNing7/minerx/internal/pkg/metrics"
	"github.com/LiangNing7/minerx/internal/pkg/util/ratelimiter"
	v1beta1 "github.com/LiangNing7/minerx/pkg/apis/apps/v1beta1"
	"github.com/LiangNing7/minerx/pkg/record"
)
...
const (
	...
	// ConfigzName is the name used for register minerx-controller manager /configz, same with GroupName.
	ConfigzName = "minerxcontrollermanager.config.liangning7.cn"
)
...
func NewControllerManagerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "minerx-controller-manager",
		Long: `The minerx controller manager is a daemon that embeds
the core control loops. In applications of robotics and
automation, a control loop is a non-terminating loop that regulates the state of
the system. In MinerX, a controller is a control loop that watches the shared
state of the rest resources through the minerx-apiserver and makes changes attempting to move the
current state towards the desired state.`,
		PersistentPreRunE: func(*cobra.Command, []string) error {
			// silence client-go warnings.
			// minerx-controller-manager generically watches APIs (including deprecated ones),
			// and CI ensures it works properly against matching minerx-apiserver version.
...
func Run(ctx context.Context, c *config.CompletedConfig) error {
	...
	*/
	...
	record.InitFromRecorder(mgr.GetEventRecorderFor("minerx-controller-manager"))
...
func CreateControllerContext(
	...
	// are ultimately the same, and the minerx-apiserver can still handle the requests correctly.
	...
	cctx := ControllerContext{
		...
		ControllerManagerMetrics:        controllersmetrics.NewControllerManagerMetrics("minerx-controller-manager"),

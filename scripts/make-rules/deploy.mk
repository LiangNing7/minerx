...
DEPLOYS ?= minerx-usercenter minerx-gateway
...
.PHONY: deploy.docker
deploy.docker:
	$(PROJ_ROOT_DIR)/scripts/installation/install.sh minerx::install::docker::install

.PHONY: deploy.docker.uninstall
deploy.docker.uninstall:
	$(PROJ_ROOT_DIR)/scripts/installation/install.sh minerx::install::docker::uninstall

.PHONY: deploy.sbs
deploy.sbs:
	$(PROJ_ROOT_DIR)/scripts/installation/install.sh minerx::install::sbs::install

.PHONY: deploy.sbs.uninstall
deploy.sbs.uninstall:
	$(PROJ_ROOT_DIR)/scripts/installation/install.sh minerx::install::sbs::uninstall

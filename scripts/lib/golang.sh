...
# To transform a string of newline-separated items to an array, use minerx::util::read-array:
# minerx::util::read-array FOO < <(minerx::golang::dups a b c a)
#
...
MINERX_GO_PACKAGE=github.com/LiangNing7/minerx
...
minerx::golang::host_platform() {
  echo "$(go env GOHOSTOS)_$(go env GOHOSTARCH)"
}

# Takes the platform name ($1) and sets the appropriate golang env variables
# for that platform.
minerx::golang::set_platform_envs() {
  [[ -n ${1-} ]] || {
    minerx::log::error_exit "!!! Internal error. No platform set in minerx::golang::set_platform_envs"
  }
  ...
  if [[ $(minerx::golang::host_platform) != "$platform" ]]; then
  ...
  ccenv=$(echo "$platform" | awk -F/ '{print "MINERX_" toupper($1) "_" toupper($2) "_CC"}')
  if [ -n "${!ccenv-}" ]; then
    export CGO_ENABLED=1
    export CC="${!ccenv}"
  fi
...
minerx::golang::verify_go_version() {
  ...
  if [[ -z "$(command -v go)" ]]; then
    minerx::log::usage_from_stdin <<EOF
...
MinerX requires ${minimum_go_version} or greater.
...
# minerx::golang::setup_env will check that the `go` commands is available in
...
minerx::golang::build_binaries_for_platform() {
 # This is for sanity.  Without it, user umasks can leak through.
  ...
  V=2 minerx::log::info "Env for ${command}/${platform}: GOOS=${GOOS-} GOARCH=${GOARCH-} GOFLAGS=${GOFLAGS-} CGO_ENABLED=${CGO_ENABLED-} CC=${CC-}"
  V=2 minerx::log::info "Env for ${command}/${platform}: GOROOT=${GOROOT-}"
  V=3 minerx::log::info "Building binaries with GCFLAGS=${gogcflags} ASMFLAGS=${goasmflags} LDFLAGS=${goldflags}"
  ...
  V=2 minerx::log::info "Output file is: ${out_dir}/${command}${ext}"
}
...
#
# Args:
#  $1: command (e.g. minerx-usercenter)
#  $2: platform (e.g. linux_amd64) (optional)
minerx::golang::build_binaries() {
  ...
  (
    ...
    local command=$1
    local host_platform=$(minerx::golang::host_platform)
    ...
    goldflags="all=$(minerx::version::ldflags) ${GOLDFLAGS:-}"
    #goldflags="all=$(minerx::version::ldflags) ${GOLDFLAGS:-}"
    if [[ "${DBG:-}" != 1 ]]; then
    ...
    minerx::golang::set_platform_envs "${platform}"
    minerx::log::status "${platform}: build started"
    minerx::golang::build_binaries_for_platform "${platform}" "${command}"
    minerx::log::status "${platform}: build finished"
  )
}

...
/*
Chain Controller Reconciliation Loop:

+-------------------------+
|  User creates Chain CR  |
+-----------+-------------+
            |
            v
+-----------+-------------+
...
type Reconciler struct {
	...
	// WatchFilterValue is the lable value used to filter events prior to reconciliation.
	WatchFilterValue string
}

func (r *Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, options controller.Options) error {
	builder := ctrl.NewControllerManagedBy(mgr).
		For(&v1beta1.Chain{}).Owns(&v1beta1.Miner{}).
		Watches(
			&v1beta1.Miner{},
			handler.EnqueueRequestsFromMapFunc(r.MinerToChains)).
		WithOptions(options).Named(controllerName).

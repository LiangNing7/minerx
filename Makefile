...
\033[35mOptions:\033[0m
  ...
  BINS             The binaries to build. Default is all of cmd.
                   ...
                   Example: make build BINS="minerx-apiserver minerx-miner-controller"
  IMAGES           Backend images to make. Default is all of cmd starting with minerx-.
                   This option is available when using: make image/image.multiarch/push/push.multiarch
                   Example: make image.multiarch IMAGES="minerx-apiserver minerx-miner-controller"
  DEPLOYS          Deploy all configured services.
  REGISTRY_PREFIX  Docker registry prefix. Default is minerxstack. 
                   Example: make push REGISTRY_PREFIX=ccr.ccs.tencentyun.com/minerxstack VERSION=v0.1.0
  PLATFORMS        The multiple platforms to build. Default is linux_amd64 and linux_arm64.
                   This option is available when using: make build.multiarch/image.multiarch/push.multiarch
                   Example: make image.multiarch IMAGES="minerx-apiserver minerx-miner-controller" PLATFORMS="linux_amd64 linux_arm64"
option go_package = "github.com/LiangNing7/minerx/pkg/api/usercenter/v1;v1";
  PLATFORMS        The multiple platforms to build. Default is linux_amd64 and linux_arm64.
                   This option is available when using: make build.multiarch/image.multiarch/push.multiarch
...
.PHONY: ca
ca: ## Generate CA files for all minerx components.
	$(MAKE) gen.ca
...
docker-install: ## Deploy minerx with docker.
	$(MAKE) deploy.docker

.PHONY: docker-uninstall
docker-uninstall: ## Deploy minerx with docker.
	$(MAKE) deploy.docker.uninstall

.PHONY:	sbs-install
sbs-install: ## Deploy minerx step by step.
	$(MAKE) deploy.sbs

.PHONY:	sbs-uninstall
sbs-uninstall: ## Deploy minerx step by step.
...
air.%: tools.verify.air
	@air -build.cmd='make build BINS=minerx-$*' -build.bin='$(OUTPUT_DIR)/platforms/$(shell go env GOOS)/$(shell go env GOARCH)/minerx-$*'

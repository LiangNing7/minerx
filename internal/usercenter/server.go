...
import (
	...
	jwtauthn "github.com/LiangNing7/goutils/pkg/authn/jwt"
	"github.com/LiangNing7/goutils/pkg/authn/jwt/store/redis"
	...
	mwjwt "github.com/LiangNing7/minerx/internal/pkg/middleware/authn/jwt"
...
func (cfg *Config) NewServer(ctx context.Context) (*Server, error) {
	...
	//nolint:contextcheck
	srv, err := InitializeWebServer(ctx.Done(),
		cfg,
		&mysqlOptions,
		cfg.JWTOptions,
		cfg.RedisOptions,
...
// NewWhiteListMatcher creates a new white list matcher.
...
func NewMiddlewares(
	...
	val validate.RequestValidator,
) []middleware.Middleware {
	// Create an OpenTelemetry meter (instrumentation scope) named "metrics"
	// to produce measurement instruments for this service.
	meter := otel.Meter("metrics")
	// Build a histogram instrument used by the metrics middleware to record
	// server-side request latency (seconds) distribution.
	seconds, _ := metrics.DefaultSecondsHistogram(meter, metrics.DefaultServerSecondsHistogramName)
	// Build a counter instrument used by the metrics middleware to count
	// the number of server-side requests handled.
...
func NewAuthenticator(jwtOpts *genericoptions.JWTOptions, redisOpts *genericoptions.RedisOptions) (authn.Authenticator, error) {
	...
	opts := []jwtauthn.Option{
		...
		jwtauthn.WithIssuer("minerx-usercenter"),

...
function minerx::util::sourced_variable {
...
function minerx::util::sortable_date() {
  date "+%Y%m%d-%H%M%S"
}

# arguments: target, item1, item2, item3, ...
# returns 0 if target is in the given items, 1 otherwise.
function minerx::util::array_contains() {
  local search="$1"
  local element
  shift
  for element; do
...
function minerx::util::wait_for_url() {
  local url=$1
  local prefix=${2:-}
  local wait=${3:-1}
  local times=${4:-30}
  ...
  command -v curl >/dev/null || {
    minerx::log::usage "curl must be installed"
    exit 1
  }
  ...
  for i in $(seq 1 "${times}"); do
    ...
    if out=$(curl --max-time "${maxtime}" -gkfs "${url}" 2>/dev/null); then
      minerx::log::status "On try ${i}, ${prefix}: ${out}"
      return 0
    fi
    sleep "${wait}"
  done
  minerx::log::error "Timed out waiting for ${prefix} to answer at ${url}; tried ${times} waiting ${wait} between each"
  return 1
}

function minerx::util::wait_for_url_with_bearer_token() {
  local url=$1
  local token=$2
  local prefix=${3:-}
  local wait=${4:-1}
  ...
  minerx::util::wait_for_url "${url}" "${prefix}" "${wait}" "${times}" "${maxtime}" -H "Authorization: Bearer ${token}"
}

# Example:  minerx::util::wait_for_success 120 5 "nodectl get nodes|grep localhost"
# arguments: wait time, sleep time, shell command
# returns 0 if the shell command get output, 1 otherwise.
function minerx::util::wait_for_success(){
  local wait_time="$1"
  local sleep_time="$2"
  local cmd="$3"
...
# Example:  minerx::util::trap_add 'echo "in trap DEBUG"' DEBUG
# See: http://stackoverflow.com/questions/3338030/multiple-bash-traps-for-the-same-signal
function minerx::util::trap_add() {
  local trap_add_cmd
  trap_add_cmd=$1
  shift
...
# Opposite of minerx::util::ensure-temp-dir()
function minerx::util::cleanup-temp-dir() {
  rm -rf "${MINERX_TEMP}"
}
...
# Vars set:
#   MINERX_TEMP
function minerx::util::ensure-temp-dir() {
  if [[ -z ${MINERX_TEMP-} ]]; then
    MINERX_TEMP=$(mktemp -d 2>/dev/null || mktemp -d -t minerx.XXXXXX)
    minerx::util::trap_add minerx::util::cleanup-temp-dir EXIT
  fi
}

function minerx::util::host_os() {
  local host_os
  case "$(uname -s)" in
    Darwin)
      host_os=darwin
    ...
    Linux)
      ...
      ;;
    *)
      minerx::log::error "Unsupported host OS.  Must be Linux or Mac OS X."
      exit 1
      ;;
  esac
  echo "${host_os}"
...
function minerx::util::host_arch() {
  local host_arch
  case "$(uname -m)" in
    x86_64*)
      host_arch=amd64
    ...
    ppc64le*)
      ...
      ;;
    *)
      minerx::log::error "Unsupported host arch. Must be x86_64, 386, arm, arm64, s390x or ppc64le."
      exit 1
      ;;
  esac
  echo "${host_arch}"
...
function minerx::util::host_platform() {
  echo "$(minerx::util::host_os)/$(minerx::util::host_arch)"
}

# looks for $1 in well-known output locations for the platform ($2)
# $PROJ_ROOT_DIR must be set
function minerx::util::find-binary-for-platform() {
  local -r lookfor="$1"
  local -r platform="$2"
  local locations=(
  ...
  if [[ "${platform}" = "$(minerx::util::host_platform)" ]]; then
    locations+=(
  ...
  if [[ -z "${bin}" ]]; then
    minerx::log::error "Failed to find binary ${lookfor} for platform ${platform}"
    return 1
  fi
...
# $PROJ_ROOT_DIR must be set
function minerx::util::find-binary() {
  minerx::util::find-binary-for-platform "$1" "$(minerx::util::host_platform)"
}
...
function minerx::util::group-version-to-pkg-path() {
...
function minerx::util::gv-to-swagger-name() {
  local group_version="$1"
  case "${group_version}" in
    v1)
      echo "v1"
...
function minerx::util::git_upstream_remote_name() {
  git remote -v | grep fetch |\
    grep -E 'github.com[/:]LiangNing7/minerx|minerxstack.io/minerx' |\
    head -n 1 | awk '{print $1}'
}

# Exits script if working directory is dirty. If it's run interactively in the terminal
# the user can commit changes in a second terminal. This script will wait.
function minerx::util::ensure_clean_working_dir() {
...
function minerx::util::base_ref() {
  ...
  full_branch="$(minerx::util::git_upstream_remote_name)/${git_branch}"
...
function minerx::util::has_changes() {
  local -r git_branch=$1
  local -r pattern=$2
  local -r not_pattern=${3:-totallyimpossiblepattern}

  local base_ref
  base_ref=$(minerx::util::base_ref "${git_branch}")
...
function minerx::util::download_file() {
  local -r url=$1
...
function minerx::util::test_openssl_installed {
...
function minerx::util::test_client_certificate_authentication_enabled {
  local output
  minerx::util::test_openssl_installed
...
function minerx::util::create_signing_certkey {
    local sudo=$1
    local dest_dir=$2
    local id=$3
    local purpose=$4
...
function minerx::util::create_client_certkey {
    local sudo=$1
    local dest_dir=$2
    local ca=$3
    local id=$4
...
function minerx::util::create_serving_certkey {
    local sudo=$1
    local dest_dir=$2
    local ca=$3
    local id=$4
...
function minerx::util::write_client_nodeconfig {
    local sudo=$1
    local dest_dir=$2
    local ca_file=$3
    local api_host=$4
...
EOF
    ...
    username=$(whoami)
    ${sudo} /usr/bin/env bash -e <<EOF
    $(minerx::util::find-binary nodectl) --nodeconfig="${dest_dir}/${client_id}.nodeconfig" config view --minify --flatten > "/tmp/${client_id}.nodeconfig"
...
function minerx::util::ensure_docker_daemon_connectivity {
...
function minerx::util::wait-for-jobs() {
  local fail=0
  local job
...
# minerx::util::join <delim> <list...>
# Concatenates the list elements with the delimiter passed as first parameter
#
# Ex: minerx::util::join , a b c
#  -> a,b,c
function minerx::util::join {
  local IFS="$1"
  shift
  echo "$*"
}
...
#
function minerx::util::ensure-cfssl {
  ...
  host_arch=$(minerx::util::host_arch)
...
# minerx::util::ensure-docker-buildx
# Check if we have "docker buildx" commands available
#
function minerx::util::ensure-docker-buildx {
...
#
function minerx::util::ensure-bash-version {
...
# minerx::util::ensure-gnu-sed
# Determines which sed binary is gnu-sed on linux/darwin
#
# Sets:
#  SED: The name of the gnu-sed binary
#
function minerx::util::ensure-gnu-sed {
  ...
  if echo "${sed_help}" | grep -q "GNU\|BusyBox"; then
    ...
    minerx::log::error "Failed to find GNU sed as sed or gsed. If you are on Mac: brew install gnu-sed." >&2
    return 1
  fi
  minerx::util::sourced_variable "${SED}"
}

# minerx::util::ensure-gnu-date
# Determines which date binary is gnu-date on linux/darwin
#
# Sets:
#  DATE: The name of the gnu-date binary
#
function minerx::util::ensure-gnu-date {
  ...
  if echo "${date_help}" | grep -q "GNU\|BusyBox"; then
    ...
    minerx::log::error "Failed to find GNU date as date or gdate. If you are on Mac: brew install coreutils." >&2
    return 1
  fi
  minerx::util::sourced_variable "${DATE}"
}

# minerx::util::check-file-in-alphabetical-order <file>
# Check that the file is in alphabetical order
#
function minerx::util::check-file-in-alphabetical-order {
...
# minerx::util::require-jq
# Checks whether jq is installed.
function minerx::util::require-jq {
  if ! command -v jq &>/dev/null; then
    minerx::log::error  "jq not found. Please install."
    return 1
  fi
}

# outputs md5 hash of $1, works on macOS and Linux
function minerx::util::md5() {
...
#
# Example usage:
#   minerx::util::read-array files < <(ls -1)
#
# When in doubt:
#  $ W=abc         # a string
#  $ X=(a b c)     # an array
...
#  $ minerx::util::read-array W < <(echo -ne "1 1\n2 2\n3 3\n")
#  bash: W is defined but isn't an array
#  $ minerx::util::read-array X < <(echo -ne "1 1\n2 2\n3 3\n")
#  $ minerx::util::read-array Y < <(echo -ne "1 1\n2 2\n3 3\n")
#  bash: Y is defined but isn't an array
#  $ minerx::util::read-array Z < <(echo -ne "1 1\n2 2\n3 3\n")
#  $ declare -p W X Y Z
#  declare -- W="abc"
#  declare -a X=([0]="1 1" [1]="2 2" [2]="3 3")
#  declare -A Y
#  declare -a Z=([0]="1 1" [1]="2 2" [2]="3 3")
function minerx::util::read-array {
  if [[ -z "$1" ]]; then
...
if [[ -z "${color_start-}" ]]; then
  ...
  minerx::util::sourced_variable "${color_start}"
  minerx::util::sourced_variable "${color_red}"
  minerx::util::sourced_variable "${color_yellow}"
  minerx::util::sourced_variable "${color_green}"
  minerx::util::sourced_variable "${color_blue}"
  minerx::util::sourced_variable "${color_cyan}"
  minerx::util::sourced_variable "${color_norm}"
fi

function minerx::util::read-available-cpus {
  if [ -z "${max_cpus}" ]; then
    max_cpus=1

...
//go:generate mockgen -destination mock_auth.go -package auth minerx/internal/usercenter/biz/v1/auth AuthBiz
...
func (b *authBiz) Login(ctx context.Context, rq *v1.LoginRequest) (*v1.LoginReply, error) {
	...
	userM, err := b.store.User().Get(ctx, where.F("username", rq.GetUsername()))
	if err != nil {
